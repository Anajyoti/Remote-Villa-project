...
function UpdateSpot() {
  ...
  const spot = useSelector((state) => state.spots.singleSpot); // Adjusted for Redux state
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(lat) || lat < -90 || lat > 90) validationErrors.lat = "Latitude must be between -90 and 90.";
    
    const lng = parseFloat(formData.lng);
    if (isNaN(lng) || lng < -180 || lng > 180) validationErrors.lng = "Longitude must be between -180 and 180.";
